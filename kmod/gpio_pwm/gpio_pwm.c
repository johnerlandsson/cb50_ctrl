...
static ssize_t set_value_callback( struct device *, struct device_attribute *, const char *, size_t );
static ssize_t get_value_callback( struct device *, struct device_attribute *, char * );
static ssize_t get_period_callback( struct device *, struct device_attribute *, char * );
...
/* set_period_callback
 * Callback funktion for sysfs write to period file
 */
				    ...
				    const char *buff, size_t count )
{
	...
	if( kstrtol( buff, 10, &period_value ) < 0 )
		return -EINVAL;
	if( period_value < 10 )
		return -EINVAL;
...
static ssize_t get_period_callback( struct device *dev, struct device_attribute *attr, char *buff )
{
	return sprintf( buff, "%d", s_PwmPeriod );
}

/* set_value_callback
 * Callback funktion for sysfs write to value file
 */
static ssize_t set_value_callback( struct device *dev, struct device_attribute *attr, 
				    const char *buff, size_t count )
{
	long value = 0;

	if( kstrtol( buff, 10, &value ) < 0 )
		return -EINVAL;
	if( value < 0 )
		return -EINVAL;
...
static ssize_t get_value_callback( struct device *dev, struct device_attribute *attr, char *buff )
{
